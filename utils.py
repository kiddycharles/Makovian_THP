...
from transformer.Encoder import get_non_pad_mask


def soft_plus(x, beta):
    temp = beta * x
    temp[temp > 20] = 20
...
def compute_integral_unbiased(model, data, time, non_pad_mask, type_mask):
    ...
    all_lambda = soft_plus(temp_hid + model.alpha * temp_time, model.beta)
...
def log_likelihood(model, data, time, types):
    ...
    all_lambda = soft_plus(all_hid, model.beta)
